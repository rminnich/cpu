...
// Package server is for building cpu servers, a.k.a. cpud A cpud is
// an ssh server with a special handler.  The handler creates a cpu
// session.  On a normal ssh session, the main task is to set up to
// run a command attached to a stdin, stdout, and stderr.
//
// The session handler is responsible for a bit more, the most
// important step being to create a private name space for the
// session.  Note that this namespace is optional; if CPU_NAMESPACE
// and CPU_FSTAB are not set, then no namespace will be set up.
// In other words, the most minimal cpu session is an ssh session.
//
...
//
// Each connection to the server results in the invocation of the commands
// send from the client. The most common command is something like:
// cpud -remote -port9p <9pportnumber> [command [arguments]].
// If there is no command, servers typically run $SHELL; that is up to whatever
// binary cpud is asked to run for each session.
//
